...
[![Reproduce](https://github.com/Asadullah-Irshad/Conformal-Trade-Execution/actions/workflows/ci.yml/badge.svg)](https://github.com/Asadullah-Irshad/Conformal-Trade-Execution/actions/workflows/ci.yml)
...
Scripts/
  run_experiments.py  # baselines + conformal + frontier + coverage  -> Results/ (Table 1)
  make_figures.py     # regenerates Figs 1–10 into Figures/
  run_real_data.py    # Figs 11–12 + Table 2 from live intraday data (yfinance)
  run_ppo.py          # train PPO over seeds, append PPO row to the table
Results/              # generated CSVs and JSON (Table 1, coverage, config)
Figures/              # all 12 paper figures (published set); Figs 1–10 regenerable via make_figures.py
...
```bash
git clone https://github.com/Asadullah-Irshad/Conformal-Trade-Execution.git
cd Conformal-Trade-Execution

python -m venv .venv && source .venv/bin/activate
pip install -r requirements.txt

python Scripts/run_experiments.py     # Table 1 + frontier + coverage -> Results/
python Scripts/make_figures.py        # regenerates Figs 1–10 -> Figures/
python Scripts/run_ppo.py --seeds 0 1 2 --timesteps 150000   # PPO comparator (needs torch)
python Scripts/run_real_data.py       # Figs 11–12 + Table 2 (needs network; see note)
```
...
- **Table 1** (simulated execution performance) → `Scripts/run_experiments.py` writes `Results/results_table.csv`.
- **Figures 1–10** (all simulator-based figures) → `Scripts/make_figures.py` writes `Figures/figure01…figure10`, regenerated from a fixed seed.
- **Figures 11–12 and Table 2** (real-data validation) → `Scripts/run_real_data.py` downloads intraday bars via `yfinance` and writes `Figures/figure11…figure12` and `Results/realdata_results_table.csv`.
...
> reproduction of the paper's Figs 11–12 / Table 2. The exact published versions
> are kept in `Figures/` (figure11, figure12) and `Tables/`. The simulator
> figures (Figs 1–10) *are* fully reproducible offline.
...
![cost–risk frontier](Figures/figure07_cost_risk_frontier.png)
...
© 2026 Asadullah Irshad and Shaon Biswas, published open access by International
Academic Press under a Creative Commons Attribution 4.0 International (CC BY 4.0)
licence; redistribution here follows that licence, with attribution as above.
